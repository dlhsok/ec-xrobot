                                    ...
                                    LOCATIONTRACKER_TYPE     goal_location )
{
  ...
  LOCATIONTRACKER_TYPE need_down_location;
              ...
              if( handle->course_speed <= handle->max_speed )
                {
                  //计算需要的减速位移(浮点运算)
                  //位移等于二分之一的加速度乘以时间的平方
                  need_down_location = (LOCATIONTRACKER_TYPE)( (float)handle->course_speed \
  ...
  //输出
  handle->go_location = (LOCATIONTRACKER_TYPE)handle->course_location;
  handle->go_speed = (LOCATIONTRACKER_TYPE)handle->course_speed;
}
